...
from verl.trainer.core_algos import agg_loss, get_policy_loss_fn, kl_penalty
...
class MegatronPPOActor(BasePPOActor):
    ...
    def make_minibatch_iterator(self, data: DataProto) -> Iterable[DataProto]:
        ...
        total_batch_size = int(data.batch.batch_size[0])
        mini_batch_size = int(self.config.ppo_mini_batch_size)
        if total_batch_size % mini_batch_size != 0:
            rollout_n = int(getattr(self.config, "rollout_n", 1) or 1)
            if total_batch_size % rollout_n != 0:
                raise AssertionError(
                    "Tail batch is not compatible with rollout repetition in megatron actor: "
                    f"{total_batch_size=} is not divisible by {rollout_n=}"
                )
            print(
                "Megatron actor dynamic mini-batch fallback: "
                f"total_batch_size={total_batch_size}, configured_mini_batch_size={mini_batch_size}"
            )
    ...
    def forward_backward_batch(
        ...
        def loss_func(output, data, meta_info):
            ...
            if forward_only:
                ...
                if self.config.use_kl_loss:
                    ref_log_prob = data["ref_log_probs"]
                    # compute kl loss
