...
def get_hf_max_position_embeddings(hf_config: PretrainedConfig) -> int:
    """Return the language-model context length from text or multimodal HF configs."""
    candidates = [hf_config]

    for attr in ("text_config", "llm_config", "language_config", "thinker_config"):
        subconfig = getattr(hf_config, attr, None)
        if subconfig is not None:
            candidates.append(subconfig)
            nested_text_config = getattr(subconfig, "text_config", None)
            if nested_text_config is not None:
                candidates.append(nested_text_config)
    ...
    raise AttributeError(f"{type(hf_config).__name__} does not expose max_position_embeddings")
...
def get_parallel_gptmodel_from_config(
    ...
    parallel_model = GPTModel(
        ...
        max_sequence_length=get_hf_max_position_embeddings(hf_config),
        pre_process=pre_process,
        post_process=post_process,
        share_embeddings_and_output_weights=share_embeddings_and_output_weights,
